 ...
 *
 * Creates non-spot layer groups (rejected spots, event notices, obstacles, protected areas)
 * and marker/layer creation functions. Spot markers are managed individually via the
 * Marker Registry and Filter Engine instead of LayerGroups.
 *
 * Reads locale from:
 * 1. data-locale attribute on the script tag
 * 2. Falls back to 'de'
 *
 * Uses window.paddelbuchMap for the map reference.
 *
...
(function() {
  ...
  // Read locale from the script tag's data-locale attribute
  var scriptTag = document.currentScript;
  var currentLocale = (scriptTag && scriptTag.getAttribute('data-locale')) || 'de';
  ...
  function initLayerControls() {
    ...
    // Read protected area type name translations from shared config (generated at build time)
    var protectedAreaTypeNames = {};
    if (window.paddelbuchMapConfig && window.paddelbuchMapConfig[currentLocale] && window.paddelbuchMapConfig[currentLocale].protectedAreaTypeNames) {
      protectedAreaTypeNames = window.paddelbuchMapConfig[currentLocale].protectedAreaTypeNames;
    } else {
      console.warn('paddelbuchMapConfig protectedAreaTypeNames not available for locale "' + currentLocale + '", using empty lookup');
    }
