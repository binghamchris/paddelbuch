/**
 * Property-Based Test: Canvas rendering for bar chart sections
 *
 * Feature: chartjs-statistics-dashboard, Property 1: Canvas rendering for bar chart sections
 *
 * For any valid statistics metrics object containing spots, obstacles, and
 * protected areas data, calling activate(context) shall produce a canvas element
 * inside the content container for each of the three bar chart sections.
 *
...
describe('Canvas rendering for bar chart sections (Property 1)', function () {
  ...
  it('produces exactly 3 canvas elements with correct data-chart-section attributes for any valid metrics', function () {
    ...
    fc.assert(
      fc.property(metricsArb, function (metrics) {
        ...
        // Verify each canvas has the correct data-chart-section attribute
        var expectedSections = ['spots', 'obstacles', 'protected-areas'];
        var foundSections = [];
        for (var i = 0; i < canvases.length; i++) {
          var section = canvases[i].getAttribute('data-chart-section');
          if (!section) {
            throw new Error(
              'Canvas at index ' + i + ' is missing data-chart-section attribute'
            );
          }
        ...
        for (var j = 0; j < expectedSections.length; j++) {
          if (foundSections.indexOf(expectedSections[j]) === -1) {
            throw new Error(
              'Missing canvas with data-chart-section="' + expectedSections[j] +
