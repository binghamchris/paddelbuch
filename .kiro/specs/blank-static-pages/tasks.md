...
- [~] 4.1 Add unit tests in `spec/contentful_mappers_spec.rb` for `map_static_page` covering: (a) rich text content as Hash with `'content'` key, (b) rich text content as object with `.content` method, (c) nil content field, (d) both `de` and `en` locales.
  - File: `spec/contentful_mappers_spec.rb`
