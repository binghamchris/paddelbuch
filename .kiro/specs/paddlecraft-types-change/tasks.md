...
- [ ] 1. Update the Dimension_Config_Generator (`_plugins/precompute_generator.rb`)
  - [ ] 1.1 Add module-level constants and ordered allow-list craft options
  ...
  - [ ] 1.2 Add localised label selection with slug fallback
  ...
  - [ ] 1.3 Add the precomputed `craft_type_display_for_locale` list
  ...
  - [ ]* 1.4 Write property test for filter option generation
  ...
  - [ ]* 1.5 Write property test for label localisation and fallback
...
- [ ] 2. Update the Craft_Icon_Include
  - [ ] 2.1 Add slug→icon cases in `_includes/craft-icon.html`
  ...
  - [ ]* 2.2 Write property test for icon mapping totality
  ...
  - [ ]* 2.3 Write icon-asset existence (regression guard) test
...
- [ ] 3. Create the Craft_Type_Display partial
  - [ ] 3.1 Implement `_includes/craft-type-display.html`
  ...
  - [ ]* 3.2 Write property test for display linked-state
...
- [ ] 4. Wire the display into Spot_Detail_Content
  - [ ] 4.1 Update `_includes/spot-detail-content.html`
  ...
  - [ ]* 4.2 Write structural/example-based tests for placement and removal
  ...
  - [ ] 4.3 Update `_tests/property/spot-detail.property.test.js` for the new display behaviour
  ...
  - [ ] 4.4 Verify the Craft_Type_Display is confined to the non-rejected partial
...
- [ ] 5. Update spot detail styles (`_sass/pages/_spot-details.scss`)
...
- [ ] 6. Update the Statistics_Dashboard craft-type icon map (`assets/js/statistics-dashboard.js`)
  - [ ] 6.1 Add the two new slug→icon mappings to `PADDLE_CRAFT_ICONS`
  ...
  - [ ]* 6.2 Write property test for dashboard icon resolution and fallback
...
- [ ] 7. Checkpoint - frontend surfaces
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 8. Create the one-time Contentful migration script (`scripts/add_paddle_craft_type_references.rb`)
  - [ ] 8.1 Implement the pure additive migration rule
  ...
  - [ ] 8.2 Implement the CLI phases modelled on `scripts/truncate_spot_location_precision.rb`
  ...
  - [ ]* 8.3 Write property test for migration rule and additivity
  ...
  - [ ]* 8.4 Write property test for no-op and no-duplicate behaviour
  ...
  - [ ]* 8.6 Write integration/mock-based tests for migration side effects
...
- [ ] 9. Verify filter engine/panel behaviour is preserved (no client code changes)
  - [ ]* 9.1 Confirm/extend the craft default-selected property test
  ...
  - [ ]* 9.2 Extend the craft set-intersection property test for the two-option set
  ...
  - [ ]* 9.3 Confirm panel-per-option and AND-across-dimensions property coverage
...
- [ ] 10. Refresh legacy craft-type slug fixtures in existing property tests
  - [ ] 10.1 Update legacy-slug fixtures to the two new slugs (representativeness only)
